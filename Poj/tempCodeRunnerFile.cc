...
string BigNumber::toString() {
    ...
    for (int i = 0; i != dot; ++i) {
        number += static_cast<char>(num_vec[i] + 30);  // '0' = 30
    }
    number += '.';
    for (unsigned i = dot; i != s; ++i) {
        number += static_cast<char>(num_vec[i] + 30);
    }
...
BigNumber pow(const BigNumber &base, unsigned exponent) {
    if (exponent == 1) {
        ...
        BigNumber res = pow(base, exponent / 2) * pow(base, exponent / 2);
        if (exponent % 2 == 1) {
            return res * base;
        }
        return res;
